...
import { Platform, StyleSheet, Text, View, ScrollView, Image, TouchableOpacity, TextInput, FlatList, ToastAndroid, ActivityIndicator } from 'react-native';
...
export default class Exhibitors extends Component {
    ...
    componentWillMount() {
        this.setState({
            ...
            this.setState({
                VisitrorData: this.props.navigation.state.params.data,
                ShowLoder: false
            }, () => {
                console.log("VisitrorData", this.state.VisitrorData);
            })
        })
    }
                    ...
                    <View style={styles.MainContainer}>
                        <View style={styles.FormContainer} >
                            ...
                            <View style={{ width: '100%', height: 'auto', backgroundColor: 'white', justifyContent: 'center', alignItems: 'center' }}>
                                {this.state.VisitrorData != [] && this.state.VisitrorData.length > 0 &&
                                    <FlatList
                                        style={{ width: '100%' }}
                                        keyExtractor={(item, index) => index}
                                        data={this.state.VisitrorData}
                                        ref={(ref) => {
                                        ...
                                        renderItem={({ item, index }) => {
                                            return (
                                                ...
                                                <TouchableOpacity onPress={()=>alert(">>>>")} style={{ width: '100%', height: 50, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', borderBottomColor: '#28A9E1', borderStyle: 'solid', borderBottomWidth: 1 }}>
